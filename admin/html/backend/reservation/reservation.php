<?php
    ...
    //-------------Pengguna--------------
    $query2 = "SELECT * FROM tb_reservasi INNER join tb_pengguna on tb_reservasi.id_pengguna=tb_pengguna.id";
    // Hubungkan query di atas ke koneksi menggunakan function prepare()
	$data2 = $connect->prepare($query2);
	// Eksekusi Query menggunakan function execute()
	$data2->execute();
	// Ubah data kedalam bentuk object
	$tampilPengguna = $data2->fetchAll(PDO::FETCH_OBJ);
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                    <div>
                        <h4 class="mb-3">Reservation list</h4>
                    </div>
                    <a href="add-reservation.php" class="btn btn-primary add-list"><i class="las la-plus mr-3"></i>Add Reservation</a>
                </div>
            </div>
            <div class="col-lg-12">
                ...
                <table class="data-table table mb-0 tbl-server-info">
                    ...
                    <tbody class="ligth-body">
                        <?php  
                            $no = 1;
                            foreach($tampilPengguna as $t) {
                                if($t->status == 'Approved'){$status = "text-success";}elseif($t->status == 'Pending'){$status = "text-warning";;}else{$status = "text-danger";;}
                                echo "
                                <tr>
                                    <td>$no</td>
                                    <td>$t->nama</td>
                                    ...
                                    <div class='d-flex align-items-center list-action'>
                                        <a class='badge bg-success mr-2' data-toggle='tooltip' data-placement='top' title='' data-original-title='Accept'
                                            href='status-reservation.php?terima=$t->no'><i class='ri-check-line mr-0'></i></a>
                                        <a class='badge bg-danger mr-2' data-toggle='tooltip' data-placement='top' title='' data-original-title='Reject'
                                            href='status-reservation.php?tolak=$t->no'><i class='ri-close-line mr-0'></i></a>
                                        <a class='badge bg-warning mr-2' data-toggle='tooltip' data-placement='top' title='' data-original-title='Delete'
                                            href='delete-reservation.php?remove=$t->no'><i class='ri-delete-bin-line mr-0'></i></a>
                                    </div>
                                </td>
                                </tr>
